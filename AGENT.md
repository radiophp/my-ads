...
- **Push notifications (PWA)**
  - The UI registers `/service-worker.js` (generated by next-pwa at build time). The push handlers live in `ui/public/push-sw.js` and are imported by the service worker.
  - If `/service-worker.js` is missing in production, the UI falls back to registering `/push-sw.js` so push can still work. Check for 200 responses on those paths when debugging.
