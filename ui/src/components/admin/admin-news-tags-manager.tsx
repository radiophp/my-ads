...
export function AdminNewsTagsManager() {
  const t = useTranslations('admin.newsTags');
  const { toast } = useToast();
  const { data: tags = [], isLoading, isFetching, refetch } = useGetAdminNewsTagsQuery();
  const [createTag, { isLoading: isCreating }] = useCreateAdminNewsTagMutation();
  const [updateTag, { isLoading: isUpdating }] = useUpdateAdminNewsTagMutation();
  const [deleteTag, { isLoading: isDeleting }] = useDeleteAdminNewsTagMutation();
