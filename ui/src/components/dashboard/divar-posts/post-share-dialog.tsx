...
import { Copy, Loader2, MessageCircle, MessageSquare, Send } from 'lucide-react';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm" hideCloseButton={false}>
        ...
        <div className="flex flex-col gap-3">
          {onShareBale && isBaleMiniApp ? (
            <Button
              type="button"
              variant="outline"
              className="flex items-center gap-2"
              disabled={shareBaleLoading}
              onClick={onShareBale}
            >
              {shareBaleLoading ? (
                <Loader2 className="size-5 animate-spin" />
              ) : (
                <BaleIcon className="size-5 text-green-600" />
              )}
              <span>{t('shareBale')}</span>
            </Button>
          ) : null}
          {onShareWhatsapp ? (
            <Button
