...
export function AdminNewsCategoriesManager() {
  const t = useTranslations('admin.newsCategories');
  const { toast } = useToast();
  const { data: categories = [], isLoading, isFetching, refetch } = useGetAdminNewsCategoriesQuery();
  const [createCategory, { isLoading: isCreating }] = useCreateAdminNewsCategoryMutation();
  const [updateCategory, { isLoading: isUpdating }] = useUpdateAdminNewsCategoryMutation();
  const [deleteCategory, { isLoading: isDeleting }] = useDeleteAdminNewsCategoryMutation();

  const [createOpen, setCreateOpen] = useState(false);
  const [editCategory, setEditCategory] = useState<NewsCategory | null>(null);
