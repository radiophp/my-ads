...
- Push delivery is handled by a PWA service worker. The UI registers `/service-worker.js` (generated by next-pwa) which imports `/push-sw.js` for push/click handlers; if `/service-worker.js` is missing, the UI falls back to registering `/push-sw.js` directly.
