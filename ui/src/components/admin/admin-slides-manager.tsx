...
import { useDeleteAdminSlideMutation, useGetAdminSlidesQuery } from '@/features/api/endpoints/slides';
...
export function AdminSlidesManager() {
  ...
  const { data, isLoading, isFetching, refetch } = useGetAdminSlidesQuery({
    page,
    pageSize,
  ...
  const [deleteSlide, { isLoading: isDeleting }] = useDeleteAdminSlideMutation();
  ...
  const openEdit = (item: Slide) => {
    router.push(`/admin/slides/${item.id}`);
  };
  ...
  const handleDelete = async (item: Slide) => {
    ...
    if (!confirmed) return;
    try {
      await deleteSlide(item.id).unwrap();
                ...
                items.map((item) => (
                  <tr key={item.id} className="border-b border-border/60 last:border-b-0">
                    <td className="py-3 pr-4">
                      <div className="relative h-12 w-24 overflow-hidden rounded-md bg-muted/50">
                    ...
                    <td className="py-3 pr-4 font-medium text-foreground">
                      {item.title || t('placeholders.untitled')}
                    </td>
                    <td className="py-3 pr-4">
                      <span
                        className={cn(
