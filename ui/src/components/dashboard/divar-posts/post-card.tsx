...
export function PostCard({
  ...
  const detailHref = href ?? (onSelect ? `/dashboard/posts/${post.id}` : null);
  const isPhotosVerified = post.photosVerified === true;
  const categoryChild = post.categoryName ?? post.categorySlug ?? null;
  const categoryParent = post.categoryParentName ?? null;
  const categoryLabel =
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-1 flex-col gap-2 pt-1">
          ...
          <div className="flex flex-col gap-2 text-sm text-muted-foreground">
            {showCategoryBreadcrumb && categoryLabel ? <span>{categoryLabel}</span> : null}
