...
const fetchBlogItem = async (slug: string): Promise<BlogItem | null> => {
  ...
  try {
    ...
    return (await response.json()) as BlogItem;
  } catch {
    return null;
  }
};
...
const fetchLatestBlogs = async (limit: number): Promise<BlogItem[]> => {
  const apiBase = await resolveApiBase();
  if (!apiBase) {
    return [];
  }
  ...
  try {
    const response = await fetch(`${apiBase}/blog?page=1&pageSize=${limit}`, {
      next: { revalidate },
    });
    if (!response.ok) {
      return [];
    }
    const data = (await response.json()) as { items?: BlogItem[] };
...
export async function generateMetadata(
  { params }: { params: BlogDetailPageProps['params'] },
  _parent: ResolvingMetadata,
): Promise<Metadata> {
  const t = await getTranslations('blog');
  const { slug } = await params;
  const item = await fetchBlogItem(slug);
  const appBase = await resolveAppBase();
  const canonicalUrl = appBase ? `${appBase}/blog/${slug}` : undefined;
...
export default async function BlogDetailPage({ params }: BlogDetailPageProps) {
  ...
  const contentBody = sourceCommentMatch
    ...
    : sourceTextMatch
      ...
      : contentValue;
  const pageUrl = appBase ? `${appBase}/blog/${item.slug}` : undefined;
  const normalizedMainImageUrl = normalizeStorageUrl(item.mainImageUrl, appBase);
  ...
  const imageUrl = toAbsoluteUrl(appBase, normalizedMainImageUrl);
  const latestBlogs = await fetchLatestBlogs(9);
  const latestItems = latestBlogs.filter((blog) => blog.id !== item.id).slice(0, 9);
  const jsonLd = {
