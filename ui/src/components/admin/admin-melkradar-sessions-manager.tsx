...
export function AdminMelkradarSessionsManager() {
  const t = useTranslations('admin.melkradarSessions');
  const { toast } = useToast();
  const { data: sessions = [], isLoading, isFetching, refetch } = useGetAdminMelkradarSessionsQuery();
  const [createSession, { isLoading: isCreating }] = useCreateAdminMelkradarSessionMutation();
  const [updateSession, { isLoading: isUpdating }] = useUpdateAdminMelkradarSessionMutation();

  const [addDialogOpen, setAddDialogOpen] = useState(false);
  const [editSession, setEditSession] = useState<AdminMelkradarSession | null>(null);
